...
void function(exports){
    ...
    var getPath = exports.getPath;
    var visibleRange = exports.visibleRange;
    var getPageSize = exports.getPageSize;
    var
    /**
     * 上报配置
     */
...
/*start hunter-group*/
    ...
    function writeGroup(type, from, to, x, y){
        ...
        if (bufferCount > maxBuffer){
            report({
                data: encode(buffer.join('') + '@@' + groupBuffer.join(''))
            });
...
/*start hunter*/
    ...
    function onIframe(/*var */iframes, frame, i, j, listener, doc, iframe){
        iframes = document.getElementsByTagName('iframe');
        for(i = 0; i < iframes.length; i++){
            iframe = iframes[i];
            try{
                var hostname = String(iframe.src).replace(/^\w+:\/\/([^\/]+).*$|.*/, '$1');
                if ((!hostname || // 非域名方式 about:blank
                    hostname == document.location.hostname) // 同域
                    && !iframe.contentWindow[eventFlag]){ // 没有绑定过
...
/*start hunter*/
        ...
        if (bufferCount > maxBuffer){
            report({
                data: encode(buffer.join('') + (reportConfig.group ? '@@' + groupBuffer.join('') : ''))
            });
            bufferCount = initBuffer;
            buffer = [];
            groupBuffer = [];
    ...
    function record(type, e, /*var */path, ts, params, range, pos, message, target){
        ...
        var ae = /^u/.test(typeof d.activeElement) ? 0 : d.activeElement
        if (ae != activeElement){ // 焦点控件变化
            writeData(['f', ts, getPath(ae)]);
            activeElement = ae;
        }
        if(type === "j") {
            writeData([type, ts].concat(params));
            return;
        }
        if (!e){
            writeData([type, ts]);
        ...
        if (lastRecordElement[0] == target){
            ...
            if (this.flag == eventFlag && this.doc){
                path = this.path + '/' + getPath(target, this.doc);
            } else {
                path = getPath(target);
            }
        }
        ...
        if ('o' == type){
            ...
            if (/[se]/.test(type)){
                ...
                if (/[mlducwr]/.test(type)){
                    //    0,  1,    2, 3, 4,  5
                    //[type, ts, path, x, y, p3]
                    pos = relPos(target, [e.clientX, e.clientY], reportConfig.grid);
                    if (!pos) return;
                    params[3] = pos[0];
                    params[4] = pos[1];
...
/*start hunter*/
        report({
            cmd: 'close',
            data: encode(buffer.join('') + 'z' + timeStamp().toString(36) + (reportConfig.group ? '@@' + groupBuffer.join('') : ''))
        });
        bufferCount = initBuffer;
        buffer = [];
        groupBuffer = [];
    ...
    function start(/*var */i, listener, ts){
        ...
        if (window['_hunter_sid']) return; // 避免冲突
        if (startTime) return;
/*end hunter*/
...
/*start hunter*/
        ...
        lastGroupRecordParams = [];
        var ae = /^u/.test(typeof d.activeElement) ? 0 : d.activeElement;
        writeData(['a', 0, vr[0], vr[1], vr[2], vr[3], getPath(ae)]);
        activeElement = ae;
        report({
            cmd: 'open',
            ref: encode(encode(d.referrer)),
            data: encode(buffer.join(''))
        });
